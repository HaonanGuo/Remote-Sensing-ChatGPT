...
class Evaluator(object):
    ...
    def F1(self):
        recall, precsion = self.Recall(), self.Precision()
        f1_1 = (2 * recall * precsion) / (recall + precsion + 1e-10)
        return f1_1
    ...
    def get_all_matrix(self):
        ...
        AA = self.Pixel_Accuracy_Class()
        ProducerA = self.Recall()
        UserA = self.Precision()
        mIoU,IoU=self.Mean_Intersection_over_Union()
        return KAPPA,OA,AA,ProducerA,UserA,mIoU,IoU