...
class InstanceSegmentation:
    ...
    def inference(self, inputs):
        ...
        return updated_image_path+f',{det_prompt} instance segmentation results have been saved'
...
class LandUseSegmentation:
    ...
    def inference(self, inputs):
        ...
        return updated_image_path+f',{det_prompt} segmentation results have been saved'
...
class RSChatGPT:
    def __init__(self,load_dict,openai_key):
    ...
    def initialize(self,gpt_name):
        self.llm = OpenAI(model_name=gpt_name,temperature=0,openai_api_key=self.openai_key)
        ...
        return gr.update(visible=True), gr.update(visible=False), gr.update(placeholder="Enter text and press enter, or upload an image"), gr.update(value="Clear")
    def run_text(self, text, state):
        ...
        return state,state
    def init_image(self, image_dir, state,txt):
        image_filename = os.path.join('image', f"{str(uuid.uuid4())[:8]}.png")
        img = io.imread(image_dir.name)
    ...
    def run_image(self, image_dir, state, txt):
        state,image_filename=self.init_image(image_dir.name,state)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--load', type=str,help='Image Captioning and Scene Classification are basic models that are required. You can select from [ObjectDetection,LandUseSegmentation,InstanceSegmentation,Image2Canny]',
                        default="ImageCaptioning_cuda:0,SceneClassification_cuda:0,ObjectDetection_cuda:0,LandUseSegmentation_cuda:0,InstanceSegmentation_cuda:0,Image2Canny_cpu")
    ...
    with gr.Blocks(css="#chatbot .overflow-y-auto{height:500px}") as demo:
        model = gr.Radio(choices = ['gpt-3.5-turbo','gpt-4'], value=None, label='OpenAI Models')
        ...
        model.change(bot.initialize, [model], [input_raws, model, txt, clear])
        txt.submit(bot.run_text, [txt, state], [chatbot, state])
        txt.submit(lambda: "", None, txt)
        btn.upload(bot.init_image, [btn, state, txt], [chatbot, state, txt])
